...
#Plug in if you are doing a chromatogram or spectra in the below quotes then fill out 
#the respective section you have chosen. 
ChromOrSpectra <- 'Chromatogram'
...
{
  #The only warning/error you should get is warning message: Removed XX rows containing missing values
  #this warning is due to us plotting a subset of our data based on our chosen x/y ranges and is no biggie
  #
  #90% of errors you can get are going to be because you either didn't put something in
  #quotes that should be, you added an extra comma in a list, Your filenames you
  #are extracting are/were typed in incorecctly, or you may have lists/dimensions of 
  # different lengths i.e. You don't your data doesn't go to your time range you
  #want, you don't have the range of ions you want, or You are plotting
  #a list of 4 chromatograms but another list you say 5 or 3 things in it (such as color list)
  #
  #Here's some example errors that do not perfectly represent all cases
  #if you get Error in c("XXXXX",'XXX',etc.) : argument XX is empty then you likely have an extra comma in 
  #one of the above lists
  #
  #If you have Error in eval(ei, envir): object 'XXX' not found. something may not be in quotes
...
###############################################################################
###############################################################################
###### Details of the Code are below! Totally ignorable but are annotated #####
###############################################################################
###############################################################################
...
if(ChromOrSpectra == 'Chromatogram' | ChromOrSpectra == 'Chrom' | ChromOrSpectra == 'chromatogram' | ChromOrSpectra == 'chrom')
  {
...
Plot
  ...
  #This is for Plotting a spectra
  
  if(ChromOrSpectra == 'Spectra' | ChromOrSpectra == 'spectra')
  {
